...
function filterLocators(category, filterType) {
  const selectedMetropolis = localStorage.getItem('selectedMetropolis') || 'All';
  const locatorsGrid = document.getElementById('locatorsGrid');

  let filteredData = locatorsData;

  // Filter by selected metropolis if not 'All'
  if (selectedMetropolis !== 'All') {
    filteredData = filteredData.filter(locator => locator.metropolis === selectedMetropolis);
  }
...
function searchServiceProviders(searchQuery) {
  ...
  const results = serviceProvidersData.filter(provider => {
    ...
    // Check for category + city combinations
    if (searchWords.includes(normalizeText(provider.category)) && 
        searchWords.includes(normalizeText(provider.metropolis))) {
      return true;
    }
...
function showProvidersOverlay() {
  ...
  const metropolises = ['All', ...new Set(serviceProvidersData.map(provider => provider.metropolis))];
        ...
        <div class="providers-filter-buttons">
          ${categories.map(category => `
            <button 
              class="filter-button ${category === 'All' ? 'active' : ''}" 
              data-category="${category}" 
              onclick="filterProviders('${category}', 'category')"
            >
              ${category}
            </button>
          `).join('')}
...
function openProviderPage(category) {
  ...
  // Filter outlets based on the selected metropolis
  const filteredOutlets = provider.details.outlets.filter(outlet => {
    return outlet.address.includes(selectedMetropolis);
  });
      ...
      <div class="tab-content">
        <div id="locator" class="tab-pane active">
          ...
          <div class="service-locator">
            <h4>Find Services in ${selectedMetropolis}</h4>
            <div class="locator-grid" id="locatorGrid">
              ${renderOutlets(filteredOutlets)}
            </div>
          </div>
...
function showMetropolisModal() {
  // Enhanced data processing with more robust error handling
  const metropolisGroups = serviceProvidersData.reduce((acc, provider) => {
    if (!provider || !provider.metropolis) return acc;
    
    const metropolis = provider.metropolis.trim();
    if (!metropolis) return acc;
    ...
    // Only add if category is unique for this metropolis
    if (provider.category && !acc[metropolis].uniqueCategories.has(provider.category)) {
      acc[metropolis].categories.add(provider.category);
      acc[metropolis].uniqueCategories.add(provider.category);
    }
    acc[metropolis].count++;
    return acc;
  }, {});
...
function selectMetropolis(metropolis) {
  ...
  // Filter providers by selected metropolis
  const filteredData = serviceProvidersData.filter(prov => 
    prov && prov.metropolis && prov.metropolis.trim() === metropolis
  );
  
  // Update the grid with filtered providers
  const providersGrid = document.getElementById('providersGrid');
  if (providersGrid) {
    ...
    announcement.setAttribute('role', 'status');
    announcement.setAttribute('aria-live', 'polite');
    announcement.textContent = `Showing services for ${metropolis}`;
    document.body.appendChild(announcement);
    setTimeout(() => announcement.remove(), 3000);
  }
...
function filterProviders(category, filterType) {
  const selectedMetropolis = localStorage.getItem('selectedMetropolis') || 'All';
  const providersGrid = document.getElementById('providersGrid');
  ...
  // Filter by selected metropolis if not 'All'
  if (selectedMetropolis !== 'All') {
    filteredData = filteredData.filter(provider => provider.metropolis === selectedMetropolis);
  }
...
function showPlacesOverlay() {
  ...
  const metropolises = ['All', ...new Set(publicPlacesData.map(place => place.metropolis))];
...
function showMetropolisModal() {
  // Enhanced data processing with more robust error handling
  const metropolisGroups = publicPlacesData.reduce((acc, place) => {
    if (!place || !place.metropolis) return acc;
    
    const metropolis = place.metropolis.trim();
    if (!metropolis) return acc;
    ...
    // Only add if category is unique for this metropolis
    if (place.category && !acc[metropolis].uniqueCategories.has(place.category)) {
      acc[metropolis].categories.add(place.category);
      acc[metropolis].uniqueCategories.add(place.category);
    }
    acc[metropolis].count++;
    return acc;
  }, {});
  ...
  modal.innerHTML = `
    <div class="metropolis-modal-content" role="document">
      <div class="metropolis-modal-header">
        <h2 id="metropolis-modal-title">Select Your City</h2>
        <p>Explore public places across different metropolises</p>
      </div>
...
function selectMetropolis(metropolis) {
  ...
  // Filter places by selected metropolis
  const filteredData = publicPlacesData.filter(place => 
    place && place.metropolis && place.metropolis.trim() === metropolis
  );
  
  // Update the grid with filtered places
  const placesGrid = document.getElementById('placesGrid');
  if (placesGrid) {
    ...
    announcement.setAttribute('role', 'status');
    announcement.setAttribute('aria-live', 'polite');
    announcement.textContent = `Showing public places for ${metropolis}`;
    document.body.appendChild(announcement);
    setTimeout(() => announcement.remove(), 3000);
  }
...
function filterPlaces(category, filterType) {
  ...
  // Filter by selected metropolis if not 'All'
  if (selectedMetropolis !== 'All') {
    filteredData = filteredData.filter(place => place.metropolis === selectedMetropolis);
  }
